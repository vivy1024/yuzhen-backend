...
| 配置项 | 值 | 说明 |
|--------|-----|------|
| **服务器** | `182.92.78.183:30932` | MySQL公网地址 |
| **用户名** | `root` | 数据库管理员账号 |
| **密码** | `build_body_2024` | 数据库密码 |
| **数据库** | `fitness_app` | 应用数据库名 |
...
#### 1. 查看表结构
```
左侧选择数据库 → 点击表名 → 查看"结构"标签
```

#### 2. 查询数据
```
左侧选择数据库 → 点击表名 → 查看"浏览"标签
或使用"SQL"标签执行自定义查询
```
...
**常见权限操作**：
- 查看用户列表：`SELECT * FROM mysql.user;`
- 授予权限：`GRANT ALL PRIVILEGES ON fitness_app.* TO 'username'@'%';`
- 刷新权限：`FLUSH PRIVILEGES;`

#### 4. 执行SQL脚本
```
顶部菜单 → SQL → 粘贴SQL语句 → 点击"执行"
```

#### 5. 导出数据
```
左侧选择数据库 → 顶部"导出"标签 → 选择格式（SQL推荐） → 点击"执行"
```

#### 6. 导入数据
```
左侧选择数据库 → 顶部"导入"标签 → 选择文件 → 点击"执行"
```
...
1. **禁止危险操作**
   - ❌ 不要执行 `DROP DATABASE` 或 `DROP TABLE` 命令
   - ❌ 不要批量删除数据（`DELETE FROM table`）
   - ❌ 不要修改核心表结构（users、exercises、foods等）

2. **操作前备份**
   - ✅ 执行重要操作前先导出数据备份
   - ✅ 使用本地备份脚本：`scripts\backup_mysql.bat`
   - ✅ 验证备份文件完整性
...
**解决方案**：
1. 检查MySQL服务状态（Zeabur控制台 → fitness_mysql）
2. 确认公网端口30932是否开放
3. 检查服务器IP是否正确（182.92.78.183）
4. 尝试重启MySQL服务
...
```sql
-- 使用root账号登录后执行
GRANT ALL PRIVILEGES ON fitness_app.* TO 'username'@'%';
FLUSH PRIVILEGES;
```
...
如果phpMyAdmin无法使用，可以使用MySQL命令行工具：

```bash
# Windows本地连接
mysql -h 182.92.78.183 -P 30932 -u root -p fitness_app

# 执行SQL文件
mysql -h 182.92.78.183 -P 30932 -u root -p fitness_app < backup.sql

# 导出数据库
mysqldump -h 182.92.78.183 -P 30932 -u root -p fitness_app > backup.sql
```
...
**推荐工具**：
- phpMyAdmin内置监控（顶部"状态"标签）
- Zeabur控制台监控面板
- MySQL Workbench（本地GUI工具）
